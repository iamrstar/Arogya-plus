...
if (!global._mongoStore) {
  global._mongoStore = {
    users: [
      ...
      { _id: "STF015", name: "Mohammad Irfan", userType: "emergency", role: "Ambulance Driver", email: "irfan@arogya.com", password: "Emergency@123", department: "Emergency" },
    ...
    appointments: [
      ...
      { _id: "APT004", patientId: "ADM001", patientName: "Arjun Mehta", doctorId: "DOC001", doctorName: "Dr. Rajesh Kumar", date: new Date().toISOString().split('T')[0], time: "02:00 PM", type: "Tele-Consultation", status: "Scheduled", meetingLink: "/patient/telemedicine?id=APT004" }
    ],
    labTests: [
    ...
    emergencyRequests: [
      { _id: "EMR001", caller: "Amit Singh", phone: "9876500001", location: "Sector 14, Main Market", type: "Cardiac Arrest", status: "Dispatched", assignedAmbulance: "AMB-01", timestamp: "2026-03-20T10:15:00Z" },
      { _id: "EMR002", caller: "Priya Das", phone: "9876500002", location: "Highway 45 Junction", type: "Accident", status: "Pending", assignedAmbulance: null, timestamp: "2026-03-20T11:00:00Z" }
    ],
...
// HOTFIX: Ensure emergency user and queue exist in active memory (bypassing HMR cache)
if (global._mongoStore) {
  if (!global._mongoStore.users.find(u => u.email === "irfan@arogya.com")) {
    global._mongoStore.users.push({ _id: "STF015", name: "Mohammad Irfan", userType: "emergency", role: "Ambulance Driver", email: "irfan@arogya.com", password: "Emergency@123", department: "Emergency" })
  }
  if (!global._mongoStore.emergencyRequests) {
    global._mongoStore.emergencyRequests = []
  }
  if (!global._mongoStore.organs) {
    global._mongoStore.organs = [
      { _id: "ORG001", type: "Kidney", bloodGroup: "O+", status: "Available", donor: "Deceased", expiryDate: new Date(Date.now() + 86400000 * 2).toISOString() },
      { _id: "ORG002", type: "Liver", bloodGroup: "A+", status: "Reserved", donor: "Living", expiryDate: new Date(Date.now() + 86400000 * 1).toISOString() }
    ]
  }
  if (!global._mongoStore.organWaitlist) {
    global._mongoStore.organWaitlist = [
      { _id: "OWL001", patientName: "Rahul Dravid", patientId: "ADM010", organNeeded: "Kidney", bloodGroup: "O+", urgencyScore: 85, status: "Waiting", dateAdded: "2026-02-10T10:00:00Z" },
      { _id: "OWL002", patientName: "Arjun Mehta", patientId: "ADM001", organNeeded: "Heart", bloodGroup: "B+", urgencyScore: 95, status: "Critical", dateAdded: "2026-03-01T08:30:00Z" }
    ]
  }
}
...
function migrateStore() {
  ...
  if (!store.bloodBank) {
    store.bloodBank = {
        inventory: [
            ...
            { bloodGroup: "A-", units: 5, lastUpdated: new Date().toISOString() },
            { bloodGroup: "B+", units: 20, lastUpdated: new Date().toISOString() },
            { bloodGroup: "B-", units: 3, lastUpdated: new Date().toISOString() },
            { bloodGroup: "AB+", units: 8, lastUpdated: new Date().toISOString() },
            { bloodGroup: "AB-", units: 2, lastUpdated: new Date().toISOString() },
            { bloodGroup: "O+", units: 25, lastUpdated: new Date().toISOString() },
            { bloodGroup: "O-", units: 6, lastUpdated: new Date().toISOString() }
        ],
        requests: []
    }
  }
  ...
  if (!store.ambulances) {
    store.ambulances = [
        { _id: "AMB-01", driver: "Mohammad Irfan", status: "Available", location: "Hospital Base", lastMaintenance: "2026-01-10" },
        { _id: "AMB-02", driver: "Rajesh Pilot", status: "On Duty", location: "Sector 45", lastMaintenance: "2026-02-15" },
        { _id: "AMB-03", driver: "Suresh Driver", status: "Maintenance", location: "Garage", lastMaintenance: "2026-03-01" },
    ]
  }

  if (!store.emergencyCases) {
    store.emergencyCases = []
  }
}
