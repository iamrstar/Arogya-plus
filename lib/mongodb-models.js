...
export async function getLabTestsByDoctor(doctorName) {
  const db = getDB()
  return db.labTests
    .filter((t) => !doctorName || t.orderedBy === doctorName)
    .sort((a, b) => new Date(b.orderDate) - new Date(a.orderDate))
}
...
export async function verifyOTP(email, otp, shouldDelete = true) {
  ...
  if (otpEntryIndex !== -1) {
    ...
    if (shouldDelete) {
      ...
      console.log(`[AA] OTP record consumed/deleted from store`)
    }
    return true
  }

  // Detailed failure logging
  const matchingEmail = db.otps.filter(o => o.email.toLowerCase() === normalizedEmail)
  if (matchingEmail.length === 0) {
    console.log(`[AA] OTP Verification FAILED: No OTP found for email ${email}`)
  } else {
    const matchingOTP = matchingEmail.find(o => o.otp === otp)
    if (!matchingOTP) {
        console.log(`[AA] OTP Verification FAILED: OTP mismatch for ${email}. Provided: ${otp}, Available OTPs for email:`, matchingEmail.map(o => o.otp))
    } else {
        const now = new Date()
        const expiry = new Date(matchingOTP.expiresAt)
        console.log(`[AA] OTP Verification FAILED: OTP expired for ${email}. Expiry: ${matchingOTP.expiresAt}, Current Time: ${now.toISOString()}, Diff: ${expiry - now}ms`)
    }
  }
  return false
}
